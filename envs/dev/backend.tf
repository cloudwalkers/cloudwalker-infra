...
terraform {
  ...
  backend "s3" {
    bucket         = "cloudwalker-terraform-state-dev" # Replace with your S3 bucket name
    key            = "us-east-1/dev/terraform.tfstate"     # Unique path for this specific project/environment's state
    region         = "us-east-1"                                # Replace with your AWS region
    encrypt        = true                                       # Ensures the state file is encrypted in S3
    dynamodb_table = "cloudwalker-terraform-dev-locks"               # Replace with your DynamoDB table name
...
resource "aws_s3_bucket" "example" {
  bucket = "cloudwalker-terraform-state-dev" # Replace with a unique bucket name
  acl    = "private"
}